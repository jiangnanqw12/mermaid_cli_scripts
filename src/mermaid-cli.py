...
def build_mermaid_command(input_path, output_path):
    """
    ...
    """
    return f'npx "C:\\001_tools\\mermaid-cli\\node_modules\\.bin\\mmdc" -i "{input_path}" -o "{output_path}"'
...
def prepare_paths(input_file_path=None):
    """
    Prepare the input and output file paths for the Mermaid CLI process.

    :param input_file_path: Path to the input Mermaid file. If None, uses a default path.
    :return: Tuple of input and output file paths.
    """
...
def main():
    ...
    input_file_path, output_file_path = prepare_paths(input_file_path_arg)
